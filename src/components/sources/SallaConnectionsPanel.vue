  ...
  <div class="flex flex-col gap-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <p class="max-w-xl text-sm text-muted">
        Salla stores that have authorized the Sfere app. Authorizing is the
        store owner's step; creating a source against the store is yours, and
    ...
    >
      ...
      <template #empty>
        <EmptyState
          title="No Salla stores have authorized yet"
          description="A store owner authorizes the Sfere app from their Salla dashboard. Once they have, the store appears here and you can create a source against it."
        >
          <template #cta>
            <SfereButton
              size="sm"
...
// `GET …/salla-connections` and `GET …/salla-authorize`. A tab on /sources
// rather than a screen, for the same reason the connector catalog is: authorizing
// a store is a step in adding a source, not something you manage on its own.
...
// Joined here by `storeId`, because `SallaConnection` carries no source id and
// `Source` carries no connection id — only a matching `store_id`.
const rows = computed(() =>
 ...
 * the merchant signs in on Salla's domain. `window.open` rather than a router
