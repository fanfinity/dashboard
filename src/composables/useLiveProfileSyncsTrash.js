...
/**
 * Soft-deleted live profile syncs.
 *
 * `public/data/trash.json` is one file keyed by resource, so this reads only its
 * `live-profile-syncs` slice — the same `select` idiom every other trash screen
 * uses. Records carry `deletedAt` / `deletedBy` / `deletedByName` on top of the
 * normal sync shape, and no delivery counters: a deleted sync delivers nothing.
 *
 ...
 *
 * @example
 * const { items, loading, error, load, restore } = useLiveProfileSyncsTrash()
 * onMounted(load)
 */
export function useLiveProfileSyncsTrash() {
  const {
    data: items,
    loading,
    error,
    ...
    select: payload => payload['live-profile-syncs']
  })
