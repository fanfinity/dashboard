...
/**
 * Soft-deleted profile DWH syncs.
 *
 * `public/data/trash.json` is one file keyed by resource, so this reads only
 * its `profile-dwh-syncs` slice — the same `select` idiom every other trash
 * screen uses. Records carry `deletedAt` / `deletedBy` / `deletedByName` on top
 * of the normal sync shape, but not the run fields: a deleted sync has stopped
 ...
 *
 * @example
 * const { items, loading, error, load, restore } = useProfileDwhSyncsTrash()
 * onMounted(load)
 */
export function useProfileDwhSyncsTrash() {
  const {
    data: items,
    loading,
    error,
    ...
    select: payload => payload['profile-dwh-syncs']
  })
