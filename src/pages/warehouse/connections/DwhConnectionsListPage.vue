...
const columns = [
  ...
  // buttons this replaced — a 180px column of whitespace is exactly what makes
...
function test(row) {
  // The "Testing…" label and the `:disabled` attribute went with the inline
  // button — a menu item is chosen and the menu closes, so it has nowhere to
  // render either. `testingId` survives as the re-entrancy GUARD alone: without
  // it, reopening the menu inside the 400ms window queues a second toast and a
  // second applyTestResult for the same row. It has no template reader any
  // more; it is not dead state.
...
// Static rather than a per-row function: unlike the other warehouse lists this
// screen has no enable/pause toggle, so neither label depends on the record.
//
// Neither item carries an icon, which is where this parts from Models and
// Syncs. There is no honest glyph for "test" in sfereIcons.js, and `play` reads
// as resume/enable everywhere else in the app; RowActionsMenu renders the glyph
// behind a `v-if`, so giving only Delete one would indent its label past the
// other's — and THAT raggedness is visible, because both items are in one menu
// at once. Two menus on two screens never are.
//
// Test opens no ConfirmDialog on purpose: the confirm rule is about state
// changes, and simulateConnectionTest opens no socket and changes no record the
// user did not already have. Delete is the state change, and it still asks.
const ROW_ACTIONS = [
  { key: 'test', label: 'Test connection' },
  { key: 'delete', label: 'Move to trash', tone: 'destructive' }
]

function onRowAction(key, row) {
  if (key === 'test') test(row)
