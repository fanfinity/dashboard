<template>
  <q-page class="p-6">
    <PageHeader
      title="Live profile syncs trash"
      subtitle="Deleted syncs are kept for 30 days, then removed for good. A sync in the trash delivers nothing."
    >
      <template #actions>
        <ToolbarSearch v-model="query" placeholder="Search trash..." />
        <button
          class="flex h-9 items-center gap-1.5 rounded-lg border border-line2 bg-white px-3 text-sm text-ink shadow-sm hover:bg-fill"
          @click="router.push({ name: 'live-profile-syncs' })"
        >
          All live syncs
        </button>
        <button
          :disabled="!items.length"
          class="flex h-9 items-center gap-1.5 rounded-lg bg-rose-600 px-3.5 text-sm font-medium text-white shadow-sm hover:opacity-90 disabled:opacity-50"
          @click="confirmEmpty = true"
        >
          Empty trash
        </button>
      </template>
    </PageHeader>

    <NoticeBanner
      v-if="expiredCount"
      variant="warn"
      title="Some of these are past their retention window"
      :message="expiredMessage"
      class="mb-5"
    />

    <DataTable
      :columns="columns"
      :rows="visible"
      :loading="loading"
      :error="error"
      row-key="id"
      :empty-title="emptyTitle"
      :empty-description="emptyDescription"
      @retry="load"
    >
      <template #cell-name="{ row }">
        <p class="font-medium text-ink">{{ row.name }}</p>
        <p class="text-xs text-subtle">{{ row.id }}</p>
      </template>

      <template #cell-audienceName="{ row }">
        <p class="text-ink">{{ row.audienceName }}</p>
        <p class="text-xs text-subtle">Keyed on {{ row.identifierTypeName }}</p>
      </template>

      <template #cell-deletedAt="{ row }">
        <p class="text-muted">{{ formatDateTime(row.deletedAt) }}</p>
        <p class="text-xs text-subtle">{{ retentionLabel(row) }}</p>
      </template>

      <template #cell-actions="{ row }">
        <div class="flex items-center justify-end gap-2">
          <button
            class="rounded-lg border border-line2 bg-white px-3 py-1.5 text-sm font-medium text-brand hover:bg-fill"
            @click.stop="onRestore(row)"
          >
            Restore
          </button>
          <button
            class="rounded-lg border border-line2 bg-white px-3 py-1.5 text-sm font-medium text-rose-600 hover:bg-fill"
            @click.stop="ask(row)"
          >
            Delete forever
          </button>
        </div>
      </template>
    </DataTable>

    <ConfirmDialog
      v-model="confirmOne"
      title="Delete forever?"
      :message="purgeMessage"
      confirm-label="Delete forever"
      destructive
      @confirm="onPurge"
    />

    <ConfirmDialog
      v-model="confirmEmpty"
      title="Empty the trash?"
      :message="emptyMessage"
      confirm-label="Empty trash"
      destructive
      @confirm="onPurgeAll"
    />
  </q-page>
</template>

<script setup>
import { computed, onMounted, ref } from 'vue'
import { useRouter } from 'vue-router'
import PageHeader from '@/components/ui/PageHeader.vue'
import DataTable from '@/components/ui/DataTable.vue'
import NoticeBanner from '@/components/ui/NoticeBanner.vue'
import ToolbarSearch from '@/components/ui/ToolbarSearch.vue'
import ConfirmDialog from '@/components/ui/ConfirmDialog.vue'
import {
  formatDateTime,
  useLiveProfileSyncToasts
} from '@/composables/useLiveProfileSyncs'
import { useLiveProfileSyncsTrash } from '@/composables/useLiveProfileSyncsTrash'

const router = useRouter()
const { toast } = useLiveProfileSyncToasts()
const { items, loading, error, load, restore, purge, purgeAll } =
  useLiveProfileSyncsTrash()

const query = ref('')
const confirmOne = ref(false)
const confirmEmpty = ref(false)
const target = ref(null)

const RETENTION_DAYS = 30
const DAY_MS = 86400000

const columns = [
  { key: 'name', label: 'Live sync', sortable: true },
  { key: 'audienceName', label: 'Audience', sortable: true },
  { key: 'profileDestinationName', label: 'Destination', sortable: true },
  { key: 'deletedAt', label: 'Deleted', sortable: true },
  { key: 'deletedByName', label: 'Deleted by', sortable: true },
  { key: 'actions', label: '', align: 'right', width: '230px' }
]

const SEARCH_FIELDS = [
  'name',
  'audienceName',
  'profileDestinationName',
  'identifierTypeName',
  'deletedByName'
]

const visible = computed(() => {
  const q = query.value.trim().toLowerCase()
  if (!q) return items.value
  return items.value.filter(i =>
    SEARCH_FIELDS.some(f =>
      String(i[f] ?? '')
        .toLowerCase()
        .includes(q)
    )
  )
})

// Days a record has left before the automatic sweep takes it.
function daysLeft(row) {
  const deleted = new Date(row.deletedAt)
  if (Number.isNaN(deleted.getTime())) return null
  return RETENTION_DAYS - Math.floor((Date.now() - deleted.getTime()) / DAY_MS)
}

function retentionLabel(row) {
  const left = daysLeft(row)
  if (left === null) return ''
  if (left <= 0) return 'Past retention — purged on the next sweep'
  return `${left} day${left === 1 ? '' : 's'} left`
}

const expiredCount = computed(
  () => items.value.filter(i => (daysLeft(i) ?? 1) <= 0).length
)

const expiredMessage = computed(() => {
  const n = expiredCount.value
  return `${n} of ${items.value.length} ${n === 1 ? 'was' : 'were'} deleted more than ${RETENTION_DAYS} days ago and will be removed on the next sweep. Restore ${n === 1 ? 'it' : 'them'} now if you still need ${n === 1 ? 'it' : 'them'}.`
})

// An empty trash is the good outcome, so it gets no call to action —
// a filtered-to-nothing trash does not need one either, it needs its query back.
const emptyTitle = computed(() =>
  items.value.length ? 'Nothing matches your search' : 'Trash is empty'
)

const emptyDescription = computed(() =>
  items.value.length
    ? 'Try a different search term.'
    : `No live profile sync has been deleted in the last ${RETENTION_DAYS} days.`
)

function onRestore(row) {
  restore(row)
  toast(`${row.name} restored, still paused`)
}

function ask(row) {
  target.value = row
  confirmOne.value = true
}

const purgeMessage = computed(() =>
  target.value
    ? `“${target.value.name}” and its delivery configuration will be removed permanently. This cannot be undone.`
    : ''
)

const emptyMessage = computed(
  () =>
    `All ${items.value.length} deleted live sync${items.value.length === 1 ? '' : 's'} will be removed permanently. This cannot be undone.`
)

function onPurge() {
  const row = target.value
  if (!row) return
  purge(row)
  toast(`${row.name} deleted permanently`)
  target.value = null
}

function onPurgeAll() {
  const count = items.value.length
  purgeAll()
  toast(`${count} live sync${count === 1 ? '' : 's'} deleted permanently`)
}

onMounted(load)
</script>
