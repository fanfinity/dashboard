<template>
  <q-page class="p-6">
    ...
    <!-- The connection health check is secondary: if it fails, the trash still
    ...
    <ConfirmDialog
      ...
      title="Restore into a failing warehouse?"
...
import { useProfileDwhSyncsTrash } from '@/composables/useProfileDwhSyncsTrash'

const router = useRouter()
const { toast } = useProfileDwhSyncToasts()
const { items, loading, error, load, restore, purge, purgeAll } =
  useProfileDwhSyncsTrash()
const {
...
const visible = computed(() => {
  ...
  return items.value.filter(i =>
    ['name', 'targetTable', 'dwhConnectionName', 'deletedByName'].some(f =>
      String(i[f] ?? '')
        .toLowerCase()
        .includes(q)
    )
...
const emptyDescription = computed(() =>
  items.value.length
    ...
    : `No profile DWH sync has been deleted in the last ${RETENTION_DAYS} days.`
)

// A missing connection cannot be checked, so it is not reported as failing —
// the check degrades to silence rather than to a false alarm.
...
const unhealthyCount = computed(() => items.value.filter(isUnhealthy).length)
...
function askRestore(row) {
  ...
  // broken one does, because the sync will silently not run.
  if (!isUnhealthy(row)) {
    onRestore(row)
    return
  }
...
const restoreMessage = computed(() =>
  target.value
    ? `${target.value.dwhConnectionName} is not accepting writes right now. “${target.value.name}” will come back paused-in-practice: it stays configured, but no run will succeed until the connection is fixed.`
    : ''
)
