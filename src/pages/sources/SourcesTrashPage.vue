...
const visible = computed(() => {
  ...
  return items.value.filter(i =>
    ['name', 'slug', 'deletedByName'].some(f =>
      String(i[f] ?? '')
        .toLowerCase()
        .includes(q)
    )
...
const emptyDescription = computed(() =>
  items.value.length
    ...
    : `No source has been deleted in the last ${RETENTION_DAYS} days.`
)
...
const purgeMessage = computed(() =>
  target.value
    ? `“${target.value.name}” and its configuration will be removed permanently. This cannot be undone.`
    : ''
)

const emptyMessage = computed(
  () =>
    `All ${items.value.length} deleted source${items.value.length === 1 ? '' : 's'} will be removed permanently. This cannot be undone.`
)
...
function onPurgeAll() {
  ...
  purgeAll()
  notifyLocal(`${count} source${count === 1 ? '' : 's'} deleted permanently`)
}
